...
// Memoized BentoCard component to prevent unnecessary re-renders
const BentoCard = memo(({ item, onClick, index }) => {
  ...
  return (
    <div
      ...
      aria-label={onClick ? `${item.title} - ${item.description}` : item.title}
      className={cn(
    ...
    >
      ...
      <div
        ...
        aria-hidden="true"
      >
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(251,146,60,0.03)_1px,transparent_1px)] bg-[length:4px_4px] will-change-opacity" />
      </div>

      <div className="relative flex flex-col space-y-3">
        <div className="flex items-center justify-between">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-white/10 group-hover:bg-orange-500/20 transition-all duration-300" aria-hidden="true">
            {item.icon}
          </div>
          {item.status && (
            <span
        ...
        <div className="flex items-center justify-between mt-auto pt-4">
          {item.tags && item.tags.length > 0 && (
            <div className="flex items-center flex-wrap gap-2 text-xs text-white/40">
              {item.tags.map((tag) => (
                <span
                  key={tag}
          ...
          <span className="text-xs text-white/40 opacity-0 group-hover:opacity-100 transition-opacity ml-auto">
            {item.cta || "Explore →"}
          </span>
        </div>
      </div>
      ...
      <div
        className={cn(
          "absolute inset-0 -z-10 rounded-xl p-px bg-gradient-to-br from-transparent via-orange-500/20 to-transparent transition-opacity duration-300",
          item.hasPersistentHover
            ? "opacity-100"
            : "opacity-0 group-hover:opacity-100"
        )}
